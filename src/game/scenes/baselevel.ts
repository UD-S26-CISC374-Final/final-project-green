...
export class baseLevel extends Scene {
    ...
    create() {
        ...
        for (let i = 0; i < this.numberOfPeople; i++) {
            ...
            while (
                this.people.some(
                    (person) =>
                        person.characterName === newPerson.characterName,
                ) ||
                this.people.some(
                    (person) => person.codename === newPerson.codename,
                ) ||
                this.people.some(
                    (person) => person.idNumber === newPerson.idNumber,
                )
            ) {
    ...
    bossTime() {
        ...
        this.currentSequence = sequences[this.bossTimeCount > 0 ? 1 : 0];
